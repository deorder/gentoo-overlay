...
# Deorder: rpi4: modified to support Raspberry Pi 4
...
IUSE="airplay alsa bluetooth bluray caps cec +css dbus dvd gbm gles lcms libressl libusb lirc mariadb mysql nfs +opengl pulseaudio raspberry-pi raspberry-pi4 samba systemd +system-ffmpeg test +udev udisks upnp upower vaapi vdpau wayland webserver +X +xslt zeroconf"
REQUIRED_USE="
	${PYTHON_REQUIRED_USE}
	|| ( gles opengl )
...
src_unpack() {
	# Deorder: rpi: Use popcornmix branch
	if use raspberry-pi; then
		EGIT_BRANCH="newclock5"
		EGIT_REPO_URI="https://github.com/popcornmix/xbmc.git"
	fi

	# Deorder: rpi4: Use popcornmix Raspberry Pi 4 hack branch
	if use raspberry-pi4; then
		EGIT_BRANCH="leia_pi4"
		EGIT_REPO_URI="https://github.com/popcornmix/xbmc.git"
	fi
